...
if os.path.exists(args.outdir+f'mcos_snr_seed_{args.process}.txt'):
    with open(args.outdir+f'mcos_snr_seed_{args.process}.txt','r') as file:
    ...
    labels = ['OS Monopole(\hat{A}^2)',
              ...
              'SNR HD',
              'Pshift Seed']
    with open(args.outdir+f'mcos_snr_seed_{args.process}.txt','w') as file:
...
if Mstart == M-1:
    ...
    for jj in range(Mstart, M):
        ...
        for ii in range(N):
            ...
            _, _, _, Asqr, Sigma = os_pshift.compute_multiple_corr_os(params=param_dict)
            Ahat_pshift.append(Asqr)
            snr_pshift.append(Asqr/Sigma)
            # if ii in check:
            #     print(f'{ii/N*100} % complete.')

        Ahat_pshift = np.array(Ahat_pshift)
        snr_pshift = np.array(snr_pshift)
        out = np.array([*np.mean(Ahat_pshift,axis=0),
                        *np.mean(snr_pshift,axis=0),
                        param_dict[seed_par]])
        with open(args.outdir+f'mcos_snr_seed_{args.process}.txt','a') as file:
